...
let Wasabi = {
  ...
  resolveTableIdx: function (tableIdx) {
    if (Wasabi.module.exports === undefined || Wasabi.module.table === undefined) {
      console.warn(
        "Wasabi: cannot resolve table index without module exports and table (possible reason: exports and table are usually not available during execution of the Wasm start function)"
      );
      return undefined;
    }
    ...
    if (resolvedFunction === null) {
      console.warn(
        "Wasabi: resolving indirectly called function failed because table returned `null` at index " +
          tableIdx
      );
      return undefined;
    }
...
// monkey-patch WebAssembly.instantiate() and .instantiateStreaming() to add Wasabi
{
    ...
    for (const hook of Wasabi.HOOK_NAMES) {
      if (Wasabi.analysis[hook] === undefined) {
        console.debug(
          "Wasabi: hook",
          hook,
          "not provided by Wasabi.analysis, I will use an empty function as a fallback"
        );
  ...
  WebAssembly.instantiate = (sourceBuffer, importObject) => {
    ...
    if (sourceBuffer instanceof WebAssembly.Module) {
