...
enum CallSite{
    Indirect_CallSite(usize, usize),
    Direct_CallSite(usize, usize),
}
    ...
    module.functions.par_iter_mut().enumerate().for_each_with(tx,|sender, (fidx, function): (usize, &mut Function)| {
        ...
        for (iidx, instr) in original_body.into_iter().enumerate() {
            ...
            if let CallIndirect(_,_) = instr {
                sender.send(CallSite::Indirect_CallSite(fidx.to_usize(), iidx)).unwrap();
            } else if let Call(_) = instr {
                sender.send(CallSite::Direct_CallSite(fidx.to_usize(), iidx)).unwrap();
            }
...
//            println!("{:?}:{:?}: {:?}", fidx.0, iidx, instr);
    ...
    let mut direct_callsites =Vec::new();
    for callsite in rx.iter(){
        match callsite{
            CallSite::Indirect_CallSite(fidx, iidx) => indirect_callsites.push((fidx, iidx)),
            CallSite::Direct_CallSite(fidx, iidx) => direct_callsites.push((fidx, iidx)),
        }
    }
    indirect_callsites.sort();
    direct_callsites.sort();
    ...
    Some((
        generate_js(module_info.into_inner(), direct_callsites, indirect_callsites,&js_hooks, node_js),
        hook_count,
    ))
}
...
fn generate_js(module_info: ModuleInfo, direct_callsites:Vec<(usize, usize)>, indirect_callsites:Vec<(usize, usize)> ,hooks: &[String], node_js: bool) -> String {
    let mut result = r#"/*
...
"#
    ...
    result.push_str("Wasabi.direct_callsites = ");
    result.push_str(&serde_json::to_string(&direct_callsites).unwrap());
    result.push_str(";\n");
    result.push_str("Wasabi.indirect_callsites = ");
    result.push_str(&serde_json::to_string(&indirect_callsites).unwrap());
    result.push_str(";\n\n");
